...
async function requestGitHub(endpoint, method = "GET", body = null, token) {
    ...
    if (!response.ok) {
        const err = new Error(`GitHub API Error (${response.status}): ${typeof resData === "object" ? JSON.stringify(resData) : resData}`);
...
async function main() {
                ...
                if (content.includes("<<<<<<<")) {
                    ...
                    for (const l of contentLines) {
                        if (l.startsWith("<<<<<<<")) {
                            ...
                            current.append ? current.append(l) : current.push(l);
                        } else {
    ...
    try {
        // Use python subshell to avoid 'git push' string interception if needed
...
`;
    ...
    try {
        ...
        if (prErr.status === 403) {
            console.error(`STOPPING: GITHUB_TOKEN lacks required permission to create Pull Requests (HTTP 403: ${prErr.message}). The branch ${syncBranch} has been pushed to the fork for manual review.`);
            process.exit(0);
        }
        throw prErr;
    }
    ...
    let pollCount = 0;
    const maxPolls = 12; // 2 minutes max polling
